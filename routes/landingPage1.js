...
router.get('/page-galleries', async (req, res) => {
    try {
        const items = await GlobalGallery.find({ portal: 'landingPage1' });
        res.json(items);
...
router.post('/page-galleries', async (req, res) => {
    try {
        ...
        const item = new GlobalGallery({ ...req.body, portal: 'landingPage1' });
        await item.save();
...
router.get('/page-sections/by-url', async (req, res) => {
    try {
        const item = await GlobalSection.findOne({ portal: 'landingPage1', urlPath: req.query.url });
        res.json(item || {});
    } catch(e) {
        res.status(500).json({error: e.message});
    }
...
router.get('/page-sections', async (req, res) => {
    try {
        const items = await GlobalSection.find({ portal: 'landingPage1' });
        res.json(items);
    } catch(e) { res.status(500).json({error: e.message}); }
});
...
router.post('/page-sections', async (req, res) => {
    try {
        const item = new GlobalSection({ ...req.body, portal: 'landingPage1' });
        await item.save();
        res.status(201).json(item);
    } catch(e) { res.status(500).json({error: e.message}); }
});
