...
router.get('/page-sections/by-url', async (req, res) => {
    try {
        const item = await GlobalSection.findOne({ portal: 'pharmacy', urlPath: req.query.url });
        res.json(item || {});
    } catch(e) {
        res.status(500).json({error: e.message});
    }
...
router.get('/page-sections', async (req, res) => {
    try {
        const items = await GlobalSection.find({ portal: 'pharmacy' });
        res.json(items);
    } catch(e) { res.status(500).json({error: e.message}); }
});
...
router.post('/page-sections', async (req, res) => {
    try {
        const item = new GlobalSection({ ...req.body, portal: 'pharmacy' });
        await item.save();
        res.status(201).json(item);
    } catch(e) { res.status(500).json({error: e.message}); }
});
